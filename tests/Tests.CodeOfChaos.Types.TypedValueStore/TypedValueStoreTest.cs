...
public class TypedValueStoreTest {
    ...
    public async Task ParallelTest_AddingLargeVolumeOfItems() {
        ...
        stopwatch.Start();
        Parallel.For(0, numberOfItems, body: i => store.TryAdd(keys[i], i));
        stopwatch.Stop();
        ...
        Parallel.For(0, numberOfItems, body: i => {
            // Yes using TUnit would have been fine here as well, but this runs a lot faster. 1s vs 6s with TUnit 
